        ...
        for k, cck in self.components.items():
                ...
                for command, inactiveset in self.commands[k].items():
                    ...
                    for tt, tank in self.instance.tanks.items():
                        ...
                        sum_inf_tank_com2= sum_inf_tank
                    ...
                    columns[(command, k)] = {'power':power[(k, command, period)], 'tank_inf': sum_inf_tank_com[command], 'tank_level': levv[command], 'ay_tank': volumes[tt, period], 'mein_tank': mein_inf[command], 'err': error_q_tot[command], 'infff': sum_inf_tank_com2, 'err_lanati': err_lanati, 'err_inf': err_inf, 'err_lev': err_lev, 'flow':jaryan}
